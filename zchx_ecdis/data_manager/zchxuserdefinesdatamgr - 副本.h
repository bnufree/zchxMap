...
class zchxUsetDefinesDataMgr : public zchxEcdisDataMgr
{
    Q_OBJECT
public:
    explicit zchxUsetDefinesDataMgr(zchxMapWidget* w, int type, QObject *parent = 0);
...
private:
    QList<std::shared_ptr<WarningZoneElement>> mData;        //告警区域数据也叫防区
};

class zchxCoastDataMgr : public zchxEcdisDataMgr
{
    Q_OBJECT
public:
    ...
    bool updateActiveItem(const QPoint &pt);
    //海岸数据
    void updateData(const QList<ZCHX::Data::ITF_CoastData> &zonelist);
    void updateData(const ZCHX::Data::ITF_CoastData &zone);
    void setData(const QList<ZCHX::Data::ITF_CoastData>& list); //设置海岸数据
    void importData(const std::vector<std::pair<double, double> > & path); //导入海岸数据
    CoastElement* item(const std::string &name);
private:
    QList<std::shared_ptr<CoastElement>> mData;        //告警区域数据也叫防区           //海岸数据
};

class zchxSeabedPipeLineMgr : public zchxEcdisDataMgr
{
    Q_OBJECT
public:
    explicit zchxSeabedPipeLineMgr(zchxMapWidget* w, QObject *parent = 0);
    void show(QPainter *painter);
    bool updateActiveItem(const QPoint &pt);
    //底管线
    void updateData(const QList<ZCHX::Data::ITF_SeabedPipeLine> &zonelist);
    void updateData(const ZCHX::Data::ITF_SeabedPipeLine &zone);
    void setData(const QList<ZCHX::Data::ITF_SeabedPipeLine> &zonelist); //设置海底管线
    void importData(const std::vector<std::pair<double, double> > & path); //导入海底管线
    SeabedPipeLineElement *item(const std::string &name);
private:
    QList<std::shared_ptr<SeabedPipeLineElement>> mData; //海底管线
};

class zchxStructureMgr : public zchxEcdisDataMgr
{
    Q_OBJECT
public:
    explicit zchxStructureMgr(zchxMapWidget* w, QObject *parent = 0);
    void show(QPainter *painter);
    bool updateActiveItem(const QPoint &pt);
    //结构物
    void updateData(const QList<ZCHX::Data::ITF_Structure> &zonelist);
    void updateData(const ZCHX::Data::ITF_Structure &zone);
    void setData(const QList<ZCHX::Data::ITF_Structure>& list); //设置结构物
    Structure *item(const std::string &name);
