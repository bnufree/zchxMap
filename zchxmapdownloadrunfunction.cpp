...
void zchxMapDownloadRunFunction::run()
{
    ...
    //计算显示位置对应的墨卡托坐标范围
    //计算当前中心经纬度对应的墨卡托坐标
    Mercator bottom_left = zchxEcdisUtils::wgs84LonlatToMercator(Wgs84LonLat(mRange.bottemLon, mRange.bottemLat ));
    Mercator top_right = zchxEcdisUtils::wgs84LonlatToMercator(Wgs84LonLat(mRange.topLon, mRange.topLat ));
